...
final class SigncryptStream extends EventEmitter implements DuplexStreamInterface
{
    private $in_buffer = '';
    ...
    public function __construct(?string $keypair, array $recipients_keys)
    {
        ...
        $this->payload_key = Signcryption::$debug_fix_key ?? random_bytes(32);

        // 2. Generate a random ephemeral keypair, using crypto_box_keypair.
        $this->ephemeral_keypair = Signcryption::$debug_fix_keypair ?? sodium_crypto_box_keypair();
        ...
        $this->sender_public_key = $this->keypair ? sodium_crypto_sign_publickey($this->keypair) : null;
        $this->sender_private_key = $this->keypair ? sodium_crypto_sign_secretkey($this->keypair) : null;

        $index = 0;
        $this->recipients = array_map(function (string $recipient_key) use (&$index) {
            return SigncryptedMessageRecipient::create(
                $recipient_key, $this->private_key, $this->payload_key, $index++
            );
        }, $recipients_keys);
    ...
    public function write($data)
    {
        ...
        while (strlen($this->in_buffer) > Signcryption::CHUNK_LENGTH) {
            $chunk = substr($this->in_buffer, 0, Signcryption::CHUNK_LENGTH);
            $this->in_buffer = substr($this->in_buffer, Signcryption::CHUNK_LENGTH);

            // This is never the final payload as there must be additional data in `in_buffer`

            $payload = SigncryptedMessagePayload::create(
                $this->header, $this->payload_key, $this->sender_private_key, $chunk,
                $this->index, /* final */ false
            );
